...
extern taskT *tasks;
bsem calcReady, waitWhileDrawing;
volatile int next;

pthread_cond_t cond_waitWhileDrawing, cond_calcReady;
pthread_mutex_t mutex_waitWhileDrawing, mutex_calcReady;
volatile int var_waitWhileDrawing = FALSE, var_calcReady = FALSE;


CCR_DECLARE(SLAVE_WAITS_THE_DRAWER);
CCR_DECLARE(DRAWER_SIGNALS_THE_SLAVE);
CCR_DECLARE(SLAVE_SIGNAL_ENDED_CALCS);
CCR_DECLARE(DRAWER_WAITS_FOR_CALCS);
...
int main(int argc, char *argv[]) {
  ...
  CCR_INIT(SLAVE_WAITS_THE_DRAWER);
  CCR_INIT(DRAWER_SIGNALS_THE_SLAVE);
  CCR_INIT(SLAVE_SIGNAL_ENDED_CALCS);
  CCR_INIT(DRAWER_WAITS_FOR_CALCS);
...
*/
  ...
  if ( mybsem_init( &waitWhileDrawing, 0) != 0)  {
    printf("Error initializing \"waitWhileDrawing\" semaphore. Exiting.\n");
    return -1;
  }
  if ( mybsem_init( &calcReady, 0) != 0)  {
    printf("Error initializing \"waitWhileDrawing\" semaphore. Exiting.\n");
    return -1;
  }
   ...
   */
   ...
   while (1) {
      ...
      */
       SLAVE_WAITS_THE_DRAWER_mode = FALSE; 
       pthread_mutex_lock(&DRAWER_SIGNALS_THE_SLAVE_mtx);
      DRAWER_SIGNALS_THE_SLAVE_mode = TRUE;
       pthread_cond_signal(&DRAWER_SIGNALS_THE_SLAVE_condvar);
       pthread_mutex_unlock(&DRAWER_SIGNALS_THE_SLAVE_mtx);

      
       
      //down( &calcReady );
       pthread_mutex_lock(&SLAVE_SIGNAL_ENDED_CALCS_mtx);
       SLAVE_SIGNAL_ENDED_CALCS_mode = TRUE;
       pthread_cond_broadcast(&SLAVE_SIGNAL_ENDED_CALCS_condvar);
       pthread_mutex_unlock(&SLAVE_SIGNAL_ENDED_CALCS_mtx);
       CCR_EXEC(DRAWER_WAITS_FOR_CALCS, (DRAWER_WAITS_FOR_CALCS_mode == TRUE),
        DRAWER_WAITS_FOR_CALCS_mode = FALSE;
       );
